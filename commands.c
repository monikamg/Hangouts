...
int cmd_exec(char* command, client_t* client, client_t** clients)
{
	...
	{
		cmd_online(client->uid,client, clients);
	}
...
void cmd_help(client_t* client, client_t** clients)
{
	...
	strcat(buff_out, "\\pm \tSend private message;\n");
        strcat(buff_out,"\\group \tSend Group message;\n");
	strcat(buff_out, "\\name \tChange nickname;\n");
...
void cmd_group(client_t* client, client_t** clients)
{
     /*char choice[56];*/ 
     char buff_out[BUFFER_SIZE];
     char msg[50];
     ...
     sprintf(buff_out,"-- 1) New Group or 2) Existing Group? -- \n");
     send_private_msg(buff_out,client->uid,clients);
     /*fgets(choice, 50 ,stdin);*/
     sprintf(msg,"New Group: Enter Users To Add: \n");
     send_private_msg(msg,client->uid,clients);
     cmd_online(client->uid,client,clients);
    ...
    for(i=0;i<MAX_CLIENTS;i++)
	{
		...
		{
		        send_group_msg_new_group(client->uid,clients);
                        continue;
		}
	}
...
void cmd_online(int uid,client_t* client, client_t** clients)
{
	char buff_out[BUFFER_SIZE];
	size_t i;
...
/*void cmd_group(client_t* client, client_t** clients)
{
     printf("New Group or Existing Group?");
}*/
